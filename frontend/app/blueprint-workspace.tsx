...
function resolveProjectImageCandidates(metadata: Record<string, any> = {}): ProjectImageCandidate[] {
  ...
  const candidates: Array<ProjectImageCandidate | null> = [
    productUrl ? { src: productUrl, label: generatedLabel } : null,
    productData ? { src: productData, label: generatedLabel } : null,
  ...
  return candidates.filter((candidate): candidate is ProjectImageCandidate => Boolean(candidate));
}
...
export function BlueprintWorkspace({ routeProjectId = null }: HomeProps = {}) {
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      if (requestId && !data?.storedVideo) {
        window.setTimeout(() => pollVideoStatus(requestId), 800);
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : "Network request failed.";
      setVideoStatus("failed");
  ...
  const projectImageCandidates = resolveProjectImageCandidates(projectIR?.assembly_metadata || {});
          ...
          <section className="min-h-0 min-w-0 flex-1 overflow-hidden">
            ...
            {activeTab === "video" && (
              <VideoPanel
                ...
                onUseProjectImage={() => {
                  setVideoImageTouched(false);
                  setVideoImageInput(defaultVideoImage);
                }}
                onRefreshGallery={() => {
                  if (currentProjectId) fetchProjectVideos(currentProjectId);
                }}
